...
async def test_completing_a_session_writes_analytics(seeded_client, mock_llm):
    ...
    # Start a session scoped to a real subject, drive the diagnostic phase, then answer.
    first = (
        await seeded_client.post(
            "/tutor/ask",
            json={"question": "How do I add 4/5 and 3/5?", "subject_id": "fractions"},
            headers=headers,
        )
    ).json()
    sid = first["session_id"]
    ...
    assert pt["subject_id"] == "fractions"
